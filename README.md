...
### Claude Code（推奨: stdio）

```sh
claude mcp add loopback -- npx loopback mcp
```

ポート管理不要、サーバー手動起動不要。Claude Codeがプロセスを自動管理します。

### Claude Desktop App（推奨: stdio）
...
packages/
  ...
  web/      — React + Vite SPA
  cli/      — CLIエントリーポイント（loopback start / mcp）
```
